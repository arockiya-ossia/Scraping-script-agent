from agent.models.trace import TraceEvent


def test_timestamp_defaults_and_varies_per_instance():
    event_a = TraceEvent(domain="example.com", node="discover", type="node_enter")
    event_b = TraceEvent(domain="example.com", node="discover", type="node_exit")
    assert event_a.timestamp > 0
    assert event_b.timestamp >= event_a.timestamp


def test_required_fields_enforced():
    event = TraceEvent(
        domain="example.com",
        node="investigate",
        type="tool_call",
        summary="probed /api/jobs",
        artifact_ref="artifacts/example.com/probe_1.json",
        tokens_prompt=120,
        tokens_completion=45,
    )
    assert event.artifact_ref.endswith("probe_1.json")
    assert event.tokens_prompt == 120


def test_extra_fields_ride_along_for_type_specific_payloads():
    event = TraceEvent(
        domain="example.com",
        node="repair_strategy",
        type="repair_decision",
        action="patch",
        failure_category="syntax_error",
    )
    dumped = event.model_dump(exclude_none=True)
    assert dumped["action"] == "patch"
    assert dumped["failure_category"] == "syntax_error"
